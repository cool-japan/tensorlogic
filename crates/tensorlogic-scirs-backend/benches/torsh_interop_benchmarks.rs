...
fn bench_tl_to_torsh(c: &mut Criterion) {
    ...
    for size in [10, 100, 1000, 10000].iter() {
        ...
        group.bench_with_input(BenchmarkId::from_parameter(size), size, |b, _| {
            b.iter(|| {
                let result = tl_to_torsh(black_box(&tensor), DeviceType::Cpu).unwrap();
                black_box(result)
            });
        });
    }
...
fn bench_tl_to_torsh_f32(c: &mut Criterion) {
    ...
    for size in [10, 100, 1000, 10000].iter() {
        ...
        group.bench_with_input(BenchmarkId::from_parameter(size), size, |b, _| {
            b.iter(|| {
                let result = tl_to_torsh_f32(black_box(&tensor), DeviceType::Cpu).unwrap();
                black_box(result)
            });
        });
    }
...
fn bench_torsh_to_tl(c: &mut Criterion) {
    ...
    for size in [10, 100, 1000, 10000].iter() {
        let data: Vec<f64> = (0..*size).map(|i| (i as f64) / (*size as f64)).collect();
        let tensor = Tensor::from_data(data, vec![*size], DeviceType::Cpu).unwrap();

        group.throughput(Throughput::Elements(*size as u64));
        group.bench_with_input(BenchmarkId::from_parameter(size), size, |b, _| {
            b.iter(|| {
                let result = torsh_to_tl(black_box(&tensor)).unwrap();
                black_box(result)
            });
        });
    }
...
fn bench_torsh_f32_to_tl(c: &mut Criterion) {
    ...
    for size in [10, 100, 1000, 10000].iter() {
        ...
        group.bench_with_input(BenchmarkId::from_parameter(size), size, |b, _| {
            b.iter(|| {
                let result = torsh_f32_to_tl(black_box(&tensor)).unwrap();
                black_box(result)
            });
        });
    }
...
fn bench_roundtrip(c: &mut Criterion) {
    ...
    for size in [10, 100, 1000, 10000].iter() {
        ...
        group.bench_with_input(BenchmarkId::from_parameter(size), size, |b, _| {
            b.iter(|| {
                let torsh = tl_to_torsh(black_box(&tensor), DeviceType::Cpu).unwrap();
                let back = torsh_to_tl(black_box(&torsh)).unwrap();
                black_box(back)
            });
        });
    }
...
fn bench_matrix_conversion(c: &mut Criterion) {
    ...
    for dim in [10, 50, 100, 200].iter() {
        let size = dim * dim;
        let data: Vec<f64> = (0..size).map(|i| (i as f64) / (size as f64)).collect();
        let tensor = ArrayD::from_shape_vec(vec![*dim, *dim], data).unwrap();
...
fn bench_hybrid_workflow(c: &mut Criterion) {
    ...
    for num_entities in [10, 50, 100, 200].iter() {
        ...
        let logic_results: Vec<f64> = (0..size)
            ...
            .collect();
        let logic_tensor = ArrayD::from_shape_vec(vec![*num_entities, *num_entities], logic_results).unwrap();
        ...
        let embeddings: Vec<f32> = (0..num_entities * embedding_size)
            ...
            .collect();
        let embedding_tensor = Tensor::from_data(embeddings, vec![*num_entities, embedding_size], DeviceType::Cpu).unwrap();

        group.throughput(Throughput::Elements(size as u64));
        group.bench_with_input(BenchmarkId::from_parameter(num_entities), num_entities, |b, _| {
            b.iter(|| {
                // Convert logic to ToRSh
                let logic_torsh = tl_to_torsh_f32(black_box(&logic_tensor), DeviceType::Cpu).unwrap();

                // Compute embedding similarity (matrix multiplication)
                let emb_t = embedding_tensor.transpose(0, 1).unwrap();
                let similarity = embedding_tensor.matmul(&emb_t).unwrap();

                // Combine logic and neural scores (hybrid reasoning)
                let alpha = 0.7_f32;
                let hybrid = logic_torsh.mul_scalar(alpha).unwrap()
                    .add(&similarity.mul_scalar(1.0 - alpha).unwrap()).unwrap();

                // Convert back to logic for constraint checking
                let result = torsh_f32_to_tl(black_box(&hybrid)).unwrap();
                black_box(result)
            });
        });
    }
...
fn bench_type_conversion(c: &mut Criterion) {
    ...
    for size in [100, 1000, 10000].iter() {
        let data: Vec<f64> = (0..*size).map(|i| (i as f64) / (*size as f64)).collect();
        let tensor = ArrayD::from_shape_vec(vec![*size], data).unwrap();
