...
fn main() {
    ...
    println!("   Compression: {:.1}x", quantized_int8_sym.memory_reduction());
    ...
    println!("   Max Absolute Error: {:.6}\n",
             (&tensor - &dequantized).iter().map(|&x| x.abs()).fold(0.0, f64::max));

    // INT8 Asymmetric Quantization
    println!("3. INT8 Asymmetric Quantization");
    println!("   ----------------------------");
    let params_int8_asym = QuantizationParams::asymmetric_per_tensor(QuantizationType::Int8, &tensor);
    ...
    println!("   Zero Point: {}", quantized_int8_asym.params.zero_point[0]);
    println!("   Memory: {} bytes", tensor.len());
    println!("   Compression: {:.1}x", quantized_int8_asym.memory_reduction());
    ...
    let sample1 = ArrayD::from_shape_vec(vec![50],
        (0..50).map(|i| (i as f64 - 25.0) * 0.05).collect()).unwrap();
    let sample2 = ArrayD::from_shape_vec(vec![50],
        (0..50).map(|i| (i as f64 - 20.0) * 0.08).collect()).unwrap();
    let sample3 = ArrayD::from_shape_vec(vec![50],
        (0..50).map(|i| (i as f64 - 30.0) * 0.06).collect()).unwrap();
    ...
    println!("   ----------------|-------------|-------------");
    println!("   INT8 Symmetric  | {:<11.1}x | {:.6}",
             quantized_int8_sym.memory_reduction(), error_int8_sym);
    println!("   INT8 Asymmetric | {:<11.1}x | {:.6}",
             quantized_int8_asym.memory_reduction(), error_int8_asym);
    println!("   INT4            | {:<11.1}x | {:.6}",
             quantized_int4.memory_reduction(), error_int4);
    println!("   FP16            | {:<11.1}x | {:.10}",
             quantized_fp16.memory_reduction(), error_fp16);
    println!("   BFloat16        | {:<11.1}x | {:.10}\n",
             quantized_bf16.memory_reduction(), error_bf16);
    ...
    println!("     Memory Saved: {:.2} GB ({:.0}%)",
             model_size_gb * (1.0 - 1.0/8.0), 87.5);

    println!("\n   With INT4 Quantization:");
    println!("     Quantized Size: {:.2} GB", model_size_gb / 16.0);
    println!("     Memory Saved: {:.2} GB ({:.0}%)",
             model_size_gb * (1.0 - 1.0/16.0), 93.75);

    println!("\n   With FP16 Quantization:");
    println!("     Quantized Size: {:.2} GB", model_size_gb / 4.0);
    println!("     Memory Saved: {:.2} GB ({:.0}%)",
             model_size_gb * (1.0 - 1.0/4.0), 75.0);
